...
def get_importances(x: tf.Tensor, model, encoder, decoder, known_class=None,
                    max_num_trials=5, entropy_reduction=0.90, jacobian_splits=1):
    ...
    z_fid = None
    x_ent = get_entropy(x, model)
    for _ in range(max_num_trials):
        z_fid = get_in_class_fiducial(x, model, encoder, decoder, known_class=known_class)
        fid_ent = get_entropy(decoder(z_fid)[0], model)
        if (fid_ent <= (1 - entropy_reduction) * x_ent):
            break

    return ig_latent_curve(z_fid, z, x, decoder, model, jacobian_splits)
...
def get_importances_LIME(x: tf.Tensor, model, mc_sample=100, num_perturb=100, alpha=0.001):
    ...
    for pert in perturbations:
        this_pert = perturb_image(x_rgb, pert, superpixels)
        pred = get_entropy(this_pert[:, :, :channels], model, mc_sample)
...
def get_importances_SHAP(x: tf.Tensor, model, x_train, mc_sample=100,
    ...
    perturbations[deac_0, deac_1, deac_2] = \
        x_train.numpy()[idx_train, deac_1, deac_2].copy()
    mask[deac_0, deac_1, deac_2] = 0

    # Score the perturbations -> in batches!
    predictions = []
    for pert in perturbations:
        pred = get_entropy(pert, model, mc_sample)
        predictions.append(pred.numpy()[0])
    predictions = np.array(predictions)
