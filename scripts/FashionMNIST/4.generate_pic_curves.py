...
def generate_attributions(dir_name: str):
    for idx in tqdm(random.sample(range(len(x_test)), k=50)):
        ...
        # Importances
        ig_entr, *_ = get_importances(x, model_drop, encoder, decoder)
        ig_entr_vanilla, *_ = get_importances_vanilla(x, model_drop)
        entr_clue = get_importances_CLUE(x, model_drop, encoder, decoder)
        entr_lime = get_importances_LIME(x, model_drop, ratio=0.4)
        entr_shap = get_importances_SHAP(x, model_drop, x_train)
...
if __name__ == "__main__":
    ...
    # Load the models
    encoder = tf.keras.models.load_model('models/vae_fashionMNIST/encoder')
    decoder = tf.keras.models.load_model('models/vae_fashionMNIST/decoder')
    model_drop = tf.keras.models.load_model('models/model_fashionMNIST')
    ...
    generate_pic_curves(attr_bins_dir_name, pic_curves_dir_name, model, "entropy", blur=3)
