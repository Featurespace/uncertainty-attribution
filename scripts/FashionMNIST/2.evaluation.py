...
def generate_attributions(
    ...
    for idx, x in enumerate(tqdm(x_test)):
        ...
        # IG
        importances['ig'][idx] = get_importances_vanilla(x, model, bins=50)
...
if __name__ == "__main__":
    ...
    x_test, y_test = ds2numpy(ds_test, max_num_batches=400) 

    # Load the models
    encoder = tf.keras.models.load_model(
        'models/vae_fashionMNIST/encoder')
    decoder = tf.keras.models.load_model(
        'models/vae_fashionMNIST/decoder')
    model = tf.keras.models.load_model('models/model_fashionMNIST')

    attr_bins_dir_name = "attr_binaries/fashion_augment"

    # Generate and save attribution maps
    os.makedirs(attr_bins_dir_name, exist_ok=True)
    generate_attributions(
        x_train, x_test, attr_bins_dir_name, model, encoder, decoder)
