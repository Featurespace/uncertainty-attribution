...
def generate_attributions(
    ...
    for idx, x in enumerate(tqdm(x_test)):
        ...
        importances['guided_ig_counter'][idx] = get_importances_guided_counter(
            x, counterfactual, model, steps=50)

        # Lime
        importances['lime'][idx] = get_importances_LIME(x, model)

        # Shap
        importances['shap'][idx] = get_importances_SHAP(x, model, x_train)
        importances['shap_counter'][idx] = get_importances_SHAP_counter(
            x, model, counterfactual)
...
def compute_performance(
        ...
        model: tf.keras.Model,
        labels: np.array,
        pixel_removal_type: str,
        blur_sd: float = 1):
    # Load importances and images
    importances = dict(
        np.load(os.path.join(binaries_dir_name, "importances_and_images.npz")))
...
def summarize_performance(binaries_dir_name: str):
    ...
    perc= pd.DataFrame(
        columns=[
            'EIC_pixel_1%', 'EIC_pixel_5%',
            'EIC_shannon_1%', 'EIC_shannon_5%',
            'EIC_spatial_1%', 'EIC_spatial_5%',],
        index=list(eic.keys()))

    for method in METHODS:
        perc.loc[method, 'EIC_pixel_1%'] = \
            (eic[method] / eic[method][-1])[::-1][
                int(np.round(eic[method].shape[0] * 0.01))]
        ...
        x_ax = ent[method] - 1
        x_ax = x_ax / x_ax[0]
        loss_target = np.where(x_ax < 0.01)[0][0]
        perc.loc[method, 'EIC_shannon_1%'] = \
    ...
    urc_perc= pd.DataFrame(
        columns=[
            'pixel_1%', 'pixel_5%',
            'shannon_1%', 'shannon_5%',
            'spatial_1%', 'spatial_5%',], index=list(eic.keys()))
    for method in METHODS:
        urc_perc.loc[method, 'pixel_1%'] = \
            urc[method][np.round(0.01*urc[method].shape[0]).astype(int)]
...
if __name__ == "__main__":
    ...
    x_train, _ = ds2numpy(ds_train, max_num_batches=400)
    x_test, y_test = ds2numpy(ds_test, max_num_batches=800) 

    # Load the models
    encoder = tf.keras.models.load_model('models/vae_MNIST/encoder')
    decoder = tf.keras.models.load_model('models/vae_MNIST/decoder')
    model = tf.keras.models.load_model('models/model_MNIST')

    # Filter instances that show some uncertainty that needs explaining
    pred_test = model.predict(x_test)
    entr_test = -np.sum(pred_test * np.log(pred_test + 1e-30), axis=-1)
    y_test = y_test[entr_test > np.median(entr_test)]
    x_test = x_test[entr_test > np.median(entr_test)]

    attr_bins_dir_name = "attr_binaries/mnist_augment"
    ...
    blur_sd = 1.5
    entr = -np.sum(model(x_test) * np.log(model(x_test)), axis=1)
    ref = gaussian_filter(x_test, sigma=(0, blur_sd, blur_sd, 0))
    entr_blur = -np.sum(model(ref) * np.log(model(ref)), axis=1)
    np.median(entr)
    np.median(entr_blur)
